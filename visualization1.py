...
st.markdown(
    ...
    <style>  
        ...
        .main .block-container { padding: 0 1rem; max-width: 100%; background-color: #f0f0f0; }  
        [data-testid="stStatusWidget"] { visibility: hidden; }  
        .main .block-container { background-color: #304666; border-radius: 10px; padding: 2rem; }  
    ...
    with col_numerical:
        ...
        with st.container():
            ...
            try:
                ...
                st.error(f"Error generating plot: category data type not found")
...
def main():
    query_params = st.experimental_get_query_params()  # Use the new st.query_params
    print(query_params)
    if "file" in query_params:
        file_path = os.path.join(
            "uploaded_files", urllib.parse.unquote(query_params["file"][0])
        )
        ...
        st.info("Please upload a file using the FastAPI endpoint.")
